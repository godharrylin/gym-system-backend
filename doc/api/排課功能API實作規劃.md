...
### 3) `[POST] /api/v1/schedule-rules/{ruleSn}`
- 目的
  - 更新規則（符合專案目前 POST 更新慣例）。
- 可更新欄位
  - `classId, dayOfWeek, startTime, durationMin, bufferMin, instructorId, isActive`。
- Application
  - 先查舊資料。
  - 套用 patch 後再次做衝堂檢查。
- Infrastructure SQL
  - `UPDATE cls_scdle_rules SET ... WHERE cls_scdle_rules_sn=@ruleSn`。
- 回傳
  - `bool` 或 `updatedRule`。
